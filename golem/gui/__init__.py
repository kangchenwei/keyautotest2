...
def login():
    ...
    if request.method == 'POST':
        ...
        if not username:
            result['errors'].append('请输入用户名')
        else:
            user_id = user.user_exists(username, root_path)
            if not user_id:
                result['errors'].append('用户名不存在')
            else:
                if not password:
                    result['errors'].append('请输入密码')
                else:
                    if not user.password_is_correct(username, password, root_path):
                        result['errors'].append('用户名或者密码不正确')
        if not next_url:
            next_url = '/'
...
# TEST SCREENSHOT UNUSED route() 装饰器把一个函数绑定到对应的 URL 上。
# TODO
